...
![多重采样展示1](./images/多重采样展示1.gif)

![多重采样展示2](./images/多重采样展示2.gif)

上面的动作图展示了启用 `centroid` 的多重采样抗锯齿效果。
下面的动作图展示了未启用 `centroid` 的多重采样抗锯齿效果。
下面这张图在边缘出会出现高亮，出现了采样错误的情况。
通过对比可以法线，启用了 `centroid` 后，可以将 uv 坐标限制在多边形内，从而避免采样错误。
