...
int main()
{
    ...
    window = glfwCreateWindow(800, 600, "Chapter19", nullptr, nullptr);
    if (!window)
    {
    ...
    torus = std::make_unique<glsl_shader::Torus>(1.75f * 0.75f, 0.75f * 0.75f, 50, 50);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glm::mat4 view = glm::lookAt(glm::vec3(5.0f, 5.0f, 7.5f), glm::vec3(0.0f, 0.75f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
        glm::mat4 projection = glm::perspective(glm::radians(60.0f), 4.0f / 3.0f, 0.3f, 100.0f);

        glm::vec4 light_position = glm::vec4(10.0f * glm::cos(angle), 3.0f, 10.0f * glm::sin(angle), 1.0f);
        program.SetUniform("u_light.position_in_view", glm::vec3(view * light_position));
        glm::mat3 normal_matrix = glm::mat3(glm::vec3(view[0]), glm::vec3(view[1]), glm::vec3(view[2]));
        program.SetUniform("u_light.direction", normal_matrix * glm::vec3(-light_position));

        program.SetUniform("u_material.Kd", 0.9f, 0.5f, 0.3f);
        program.SetUniform("u_material.Ks", 0.95f, 0.95f, 0.95f);
        program.SetUniform("u_material.Ka", 0.9f * 0.3f, 0.5f * 0.3f, 0.3f * 0.3f);
...
{
    // 创建顶点着色器
    program.CompileShader("../../assets/shaders/chapter19/cartoon_effect.vs.glsl");
    program.CompileShader("../../assets/shaders/chapter19/cartoon_effect.fs.glsl");
    program.Link();
    program.Use();
    ...
    program.SetUniform("u_light.exponent", 50.0f);
    program.SetUniform("u_light.cut_off", glm::radians(15.0f));
}