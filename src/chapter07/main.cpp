...
int main()
{
    ...
    window = glfwCreateWindow(800, 600, "Chapter07", nullptr, nullptr);
    if (!window)
    {
...
{
    // 创建顶点着色器
    program.CompileShader("../../assets/shaders/chapter07/phong_diffuse.vs.glsl");
    program.CompileShader("../../assets/shaders/chapter07/phong_diffuse.fs.glsl");
    program.Link();
    program.Use();
...
void Update()
{
    ...
    program.SetUniform("u_Ld", 0.9f, 0.5f, 0.3f);
    program.SetUniform("u_Kd", 1.0f, 1.0f, 1.0f);
    program.SetUniform("u_light_position_in_view", view * glm::vec4(5.0f, 5.0f, 2.0f, 1.0f));
}