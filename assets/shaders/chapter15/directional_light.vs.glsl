...
vec3 CalculatePhongModel(vec3 position, vec3 normal)
{
    ...
    vec3 diffuse_color = u_light.L * u_material.Kd * s_dot_n;
    ...
    if(s_dot_n > 0.0)
    {
        ...
        specular_color = u_light.L * u_material.Ks * pow(max(dot(r, v), 0.0), u_material.shininess);
    }
...
void main()
{
    ...
    color = CalculatePhongModel(view_position, view_normal);
